...
import { ArrowDown, Menu, X, Droplets, Map, Zap, Layers } from 'lucide-react';
...
const AuthorCard = ({ name, role, university, index }: { name: string, role: string, university: string, index: number }) => {
  return (
    <motion.div 
      ...
      className="flex flex-col group items-start p-6 bg-white rounded-xl border border-slate-200 shadow-sm hover:shadow-lg transition-all duration-300 w-full md:w-[calc(50%-1rem)] lg:w-[calc(33%-1rem)] hover:border-ocean"
    >
      <h3 className="font-serif text-xl text-slate-900 mb-2">{name}</h3>
...
const Website: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 selection:bg-ocean selection:text-white">
      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'bg-white/90 backdrop-blur-md shadow-sm py-4' : 'bg-transparent py-6'}`}>
        <div className="container mx-auto px-6 flex justify-between items-center">
          <div className="flex items-center gap-3 cursor-pointer" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
            ...
            <span className={`font-serif font-bold text-xl tracking-tight transition-opacity ${scrolled ? 'text-slate-900' : 'text-slate-900'}`}>
              USS-WATER <span className="font-sans font-light text-slate-500 text-sm ml-1">2024</span>
            </span>
          </div>
          
          <div className="hidden md:flex items-center gap-8 text-sm font-medium tracking-wide text-slate-600">
            <a href="#intro" onClick={scrollToSection('intro')} className="hover:text-ocean transition-colors uppercase">Overview</a>
            <a href="#dataset" onClick={scrollToSection('dataset')} className="hover:text-ocean transition-colors uppercase">Dataset</a>
            <a href="#model" onClick={scrollToSection('model')} className="hover:text-ocean transition-colors uppercase">U-Net+ Model</a>
            <a href="#results" onClick={scrollToSection('results')} className="hover:text-ocean transition-colors uppercase">Results</a>
            <a 
              href="https://github.com/NischalRam/USS--Water-Remote-Sensing-for-Water-Detection" 
              target="_blank" 
              rel="noopener noreferrer" 
              className="px-5 py-2 bg-slate-900 text-white rounded-full hover:bg-ocean transition-colors shadow-sm"
            >
              Get Data
            </a>
          </div>
      ...
      <header className="relative h-screen flex items-center justify-center overflow-hidden bg-slate-900">
        ...
        {/* Gradient Overlay */}
        <div className="absolute inset-0 z-0 pointer-events-none bg-gradient-to-b from-transparent via-slate-900/20 to-slate-900/90" />
      ...
      <main>
        ...
        <section id="intro" className="py-24 bg-white">
          <div className="container mx-auto px-6 md:px-12 grid grid-cols-1 md:grid-cols-12 gap-12 items-start">
            <Reveal className="md:col-span-4">
              ...
              <h2 className="font-serif text-4xl mb-6 leading-tight text-slate-900">Why Map Water?</h2>
              <div className="w-20 h-1.5 bg-gradient-to-r from-ocean to-cyan-300 mb-6 rounded-full"></div>
            </Reveal>
            <Reveal className="md:col-span-8 text-lg text-slate-600 leading-relaxed space-y-6" delay={0.2}>
              <p>
              ...
              <div className="p-6 bg-slate-50 border-l-4 border-ocean rounded-r-xl">
        ...
        {/* The Dataset */}
        <section id="dataset" className="py-24 bg-slate-50 border-y border-slate-200">
            <div className="container mx-auto px-6">
                <Reveal className="text-center mb-16">
                    <div className="inline-flex items-center gap-2 px-3 py-1 bg-white text-slate-600 text-xs font-bold tracking-widest uppercase rounded-full mb-4 border border-slate-200 shadow-sm">
                        <Map size={14}/> The Data
                    </div>
                    <h2 className="font-serif text-4xl md:text-5xl mb-4 text-slate-900">USS-Water Dataset</h2>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
                    ...
                    <div className="order-1 lg:order-2 space-y-8">
                        <Reveal delay={0.1}>
                            <h3 className="text-2xl font-serif text-slate-900 mb-2">Unprecedented Scale</h3>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            {[
                                ...
                                <Reveal key={i} className="p-4 bg-white rounded-lg shadow-sm border border-slate-100" delay={0.1 + (i * 0.1)}>
                        ...
                        <Reveal delay={0.3}>
                            <h3 className="text-lg font-bold text-slate-900 mb-3 mt-4">Water Classes</h3>
                            <div className="flex flex-wrap gap-2">
                                {['Rivers', 'Lakes', 'Ponds', 'Reservoirs', 'Wetlands', 'Creeks', 'Coastal'].map((c) => (
                                    <span key={c} className="px-3 py-1 bg-cyan-100 text-cyan-800 rounded-full text-sm font-medium border border-cyan-200">
                                        {c}
                                    </span>
                                ))}
                            </div>
        ...
        {/* The Model: U-Net+ */}
        <section id="model" className="py-24 bg-slate-900 text-white overflow-hidden relative">
            ...
            <div className="container mx-auto px-6 relative z-10">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
                     <Reveal>
                        <div className="inline-flex items-center gap-2 px-3 py-1 bg-slate-800 text-cyan-400 text-xs font-bold tracking-widest uppercase rounded-full mb-6 border border-slate-700">
                     ...
                     <Reveal delay={0.2} className="bg-slate-800/50 p-2 rounded-2xl border border-slate-700 shadow-2xl">
                        <ArchitectureDiagram />
                     </Reveal>
                </div>
            </div>
        ...
        <section id="results" className="py-24 bg-white">
            <div className="container mx-auto px-6">
                <Reveal className="max-w-4xl mx-auto text-center mb-16">
                    <h2 className="font-serif text-4xl md:text-5xl mb-6 text-slate-900">Performance Benchmarks</h2>
                ...
                <Reveal delay={0.2} className="bg-white rounded-2xl shadow-xl border border-slate-100 overflow-hidden">
                    <PerformanceChart />
                </Reveal>

                <div className="mt-16 grid grid-cols-1 md:grid-cols-2 gap-8">
                     <Reveal delay={0.3} className="p-8 bg-slate-50 rounded-xl border border-slate-200">
                        <h3 className="font-serif text-2xl text-slate-900 mb-4">Urban Case Study: Chicago</h3>
                     ...
                     <Reveal delay={0.4} className="p-8 bg-slate-50 rounded-xl border border-slate-200">
                        <h3 className="font-serif text-2xl text-slate-900 mb-4">Industrial Case Study: Gary, IN</h3>
                        <p className="text-slate-600 mb-4">
        ...
        <section id="authors" className="py-24 bg-slate-100 border-t border-slate-200">
           <div className="container mx-auto px-6">
                <Reveal className="text-center mb-12">
                    <div className="inline-block mb-3 text-xs font-bold tracking-widest text-slate-500 uppercase">THE TEAM</div>
                    <h2 className="font-serif text-3xl md:text-5xl mb-4 text-slate-900">Research Authors</h2>
                    <p className="text-slate-500 max-w-2xl mx-auto">Northern Illinois University</p>
                </Reveal>
                
                <div className="flex flex-wrap gap-6 justify-center">
                    <AuthorCard name="Nischal Vooda" role="Lead Author" university="Dept. of Computer Science, NIU" index={0} />
                    <AuthorCard name="Mohammed Sohaib Uddin" role="Co-Author" university="College of Business, NIU" index={1} />
                    <AuthorCard name="Abdul Rahman Shaikh" role="Co-Author" university="Dept. of Computer Science, NIU" index={2} />
                    <AuthorCard name="Mani Sai Lakshmi Karasani" role="Co-Author" university="Dept. of Computer Science, NIU" index={3} />
                    <AuthorCard name="Madhu G. R. Ambati" role="Co-Author" university="Dept. of Computer Science, NIU" index={4} />
        ...
        <div className="container mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-8">
            <div className="text-center md:text-left">
                <div className="text-white font-serif font-bold text-2xl mb-2 flex items-center gap-2 justify-center md:justify-start">
                    <Droplets size={24} className="text-ocean" /> USS-Water
                </div>
